...
```{r}
df2 %>% kbl() %>% kable_paper() %>% scroll_box(width = "1500px", height = "800px")
```

# 분석 3

```{r}
df3 %>% kbl() %>% kable_paper() %>% scroll_box(width = "1500px", height = "800px")
```
...
data %>%
  ...
  geom_errorbar(aes(ymin = TEWL_Mean, ymax = TEWL_Mean), width = 0.5, data = data %>% group_by(`Measurement Time`) %>% summarise(TEWL_Mean = mean(TEWL_Mean))) +
  scale_shape_manual("legend", values = c("30s" = 1, "1m" = 2, "2m" = 5,"4m" = 6)) +
  labs(x ="Time", y = "TEWL") +
  ylim(c(0, 50)) +
  theme(legend.position = "none")
```
...
data %>%
  ...
  geom_point(aes(shape = Time)) +
  geom_errorbar(aes(ymin = SH_Mean, ymax = SH_Mean), width = 0.5, data = data %>% group_by(`Measurement Time`) %>% summarise(SH_Mean = mean(SH_Mean))) +
  scale_shape_manual("legend", values = c("30s" = 1, "1m" = 2, "2m" = 5,"4m" = 6)) +
  labs(x = "Time", y = "SH") +
  ylim(c(0, 60)) +
  theme(legend.position = "none")
...
data %>%
  ...
  geom_point(aes(shape = Time)) +
  geom_errorbar(aes(ymin = Tem_Mean, ymax = Tem_Mean), width = 0.5, data = data %>% group_by(`Measurement Time`) %>% summarise(Tem_Mean = mean(Tem_Mean))) +
  scale_shape_manual("legend", values = c("30s" = 1, "1m" = 2, "2m" = 5,"4m" = 6)) +
  labs(x = "Time", y = "Tem") +
  ylim(c(34, 40)) +
  theme(legend.position = "none")
